...
local function load_acls_no_route_into_memory(route_id)
    local results, err = kong.db.acls_no_routes:find_all { route_id = route_id }
    if err then
        return nil, err
    end
    return results
...
--     local cache_key_no_route = kong.db.acls_no_routes:cache_key(route_id)
...
function ACLHandler:access(conf)
    ...
    if cached_block == true then
        -- NOTE: we only catch the boolean here!
        -- return responses.send_HTTP_FORBIDDEN("You cannot consume this service")
        return kong.response.error(403, "You cannot consume this service")
    end
